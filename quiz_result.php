...
<main class="cd-main-content">
    ...
    <div class="row">
        <div class="col">
            <h1 class="item-title-hvy">The right rice for you is...</h1>
            <div class="grid-container" id="products">
                <?php
                   // echo "$sticky<br>$tender<br>$cook<br>$color<br>";
                    $sql = "SELECT * FROM prods WHERE sticky='$sticky' and tender = '$tender' and cook = '$cook' and color = '$color'";
                    $result = $conn->query($sql);
                    while($rows = $result->fetch_assoc()){
                        if($rows['prod_stock']<=5){ $st= 'disabled';}
                        echo '<div class="item1">
                                    ...
                                    <form method="post" class="btn-form">
                                        <button class="add-to-cart" id="products-btn" type="submit" name="sub" value='.$rows['prod_id'].' '.$st.'>'.$rows['prod_price'].'/kg <span class="price-btn">+</span></button>s
                                    </form>
                            </div>';
